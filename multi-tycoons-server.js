...
var isExistingRoom = function(parRoom){
	...
	for (var i=0; i<rooms.length; i++){
		console.log("isExistingRoom: " + rooms[i].roomname + " = " + parRoom);
		if (rooms[i].roomname == parRoom) { return rooms[i]; }

	}
	console.log("isExistingRoom: " + parRoom + " not found");
	return false;
...
var findPlayerByConnection = function(parConnection){
	...
	console.log("findPlayerByConnection " + parConnection.remoteAddress + " | total players: " + players.length);
	for (var i=0; i<players.length; i++){
		console.log("player " + i + ": " + players[i].playername + " " + players[i].connection.remoteAddress);
		if (players[i].connection == parConnection) { return players[i]; }

	}
	console.log("findPlayerByConnection: player connection not found");
	return false;
    ...
    connection.on('message', function(message) {
		...
		case 'create_room': // checks if the room already exists
		  if (isExistingRoom(packet_in.data)) {
			  ...
			  sendMessage("system_message", "room " + packet_in.data + " created!", connection);
		  }

		case 'join_room':
			  var searchedRoom = joinRoom(packet_in.data, connection);
			  if (searchedRoom != false) { 
				sendMessage("system_message", "you have joined room " + packet_in.data, connection);
				searchedRoom.showPlayersInRoom();
			  }
			  else { sendMessage("system_message", "unable to join room " + packet_in.data, connection); }
		  break;
		case 'leave_room':
			var index = isExistingRoom(packet_in.data);
			if (index != false) {
				rooms[index].RemovePlayer("connection",connection);	
			}
		default:
		};
		...
		joinRoom("Lobby", connection);
		sendMessage("system_message", "you have joined the Lobby", connection);
